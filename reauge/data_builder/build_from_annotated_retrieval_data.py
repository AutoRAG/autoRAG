...
def generate_qa_pairs_from_annotated_pairs(
    ...
    for question, annotated_text in tqdm(annotated_pairs):
        best_matched_node_score = max([(node, fuzz.partial_ratio(annotated_text, node.text)) for node in nodes], key=lambda x: x[1])
...
def main(cfg: DictConfig):
    ...
    annotated_data_path = cfg.data_builder.build_from_annotated_retrieval_data.annotated_data_path
    ...
    annotated_field = cfg.data_builder.build_from_annotated_retrieval_data.annotated_field
    ...
    qa_data_from_annotated = generate_qa_pairs_from_annotated_pairs(nodes, annotated_pairs)
